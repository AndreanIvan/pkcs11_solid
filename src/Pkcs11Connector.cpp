...
CK_RV Pkcs11Connector::openSession() {
    ...
    CK_RV rv = C_GetSlotList(CK_TRUE, slotList, &slotCount);
    if (rv != CKR_OK) {
...
CK_RV Pkcs11Connector::closeSession() {
    // Close the session
    CK_RV rv = C_CloseSession(session);
    if (rv != CKR_OK) {
...
CK_RV Pkcs11Connector::login(const char* pin) {
    // Login to the token
    CK_RV rv = C_Login(session, CKU_USER, (CK_UTF8CHAR_PTR)pin, strlen(pin));
    if (rv != CKR_OK) {
...
CK_RV Pkcs11Connector::generateKey(CK_ULONG keySize) {
    ...
    CK_RV rv = C_GenerateKey(session, &keyGenMech, keyTemplate, 6, &aesKey);
    if (rv != CKR_OK) {
